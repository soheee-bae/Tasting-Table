...
export const getBookmarksByUserId = async (props: BookmarkIdProps) => {
  const { id } = props;
  try {
    const bookmarks = await axios.get(`https://tasting-table.onrender.com/bookmark/${id}`);
...
export const addBookmark = async (props: BookmarkProps) => {
  try {
    const bookmark = await axios.post('https://tasting-table.onrender.com/bookmark/', {
      ...props
    });
...
export const deleteBookmark = async (props: BookmarkIdProps) => {
  ...
  try {
    const bookmark = await axios.delete(`https://tasting-table.onrender.com/bookmark/${id}`);
